...
#    print timeList, 
#    if int(timeList[3]) < 10:
#        
#        currentMDY = str(timeList[1]) + ' ' + str('0') + str(timeList[3]) + ' ' + str(timeList[5])
#    else:
#        currentMDY = str(timeList[1]) + ' ' + str(timeList[3]) + ' ' + str(timeList[5])
...
#        ''
    ...
    if wx.location == 'Unknown Station':
        'unknown station'
...
for wx in cleanWeatherDataList:
    ...
    'properties': {'temp_f': str(wx.tempF), 'wind':wx.windMPH,'windGust':wx.windGust ,'humidity':wx.humidity,'pressure':wx.pressure,'visibility':wx.visibility,'marker-symbol':'default_marker'}}
    print d1, 'd1'
    jsonData.append(d1)

jsonData = {'type': 'FeatureCollection', 'features': jsonData}
#jsonData = 'var jsonData =' + str(jsonData)
